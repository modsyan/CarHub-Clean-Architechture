...
namespace Mac.CarHub.Infrastructure.Files;
...
public class FileService : IFileService
{
    ...
    public FileService(IWebHostEnvironment webHostEnvironment, IOptions<FileOptions> fileOptions,
        IStringLocalizer<FileService> localizer, IApplicationDbContext context)
    {
        ...
        _uploadsFolder = $"{_webHostEnvironment.WebRootPath}{_fileOptions.Value.WwwRootImagePath}";
    ...
    public async Task<UploadedFile> SaveToDiskAsync(IFormFile file, CancellationToken cancellationToken = default)
    {
        if (file.Length <= 0) throw new ArgumentException(_localizer[SharedResourcesKeys.ERR_FILE_PROVIDED_NOT_VALID]);

        var fileId = Guid.NewGuid();
        var genFileName = $"{fileId}{Path.GetExtension(file.FileName)}";
        var filePath = Path.Combine(_uploadsFolder, genFileName);
        ...
        var uploadedFile = new UploadedFile()
        {
            ...
            FilePath = Path.Combine(_fileOptions.Value.WwwRootImagePath, genFileName),
...
#pragma warning disable CS1998 // Async method lacks 'await' operators and will run synchronously
    public async Task<UploadedFile> SaveToDiskAsync(byte[] file, string fileName,
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        CancellationToken cancellationToken = default)
    {
        // the format of the file is where is an base64 
        ...
        //     Guard.Against.InvalidFormat(file, localizer[SharedResourcesKeys.ERR_MAX_FILE_SIZE, 5], null, null);
        
        
        if (file.Length <= 0) 
            throw new ArgumentException(_localizer[SharedResourcesKeys.ERR_FILE_PROVIDED_NOT_VALID]);
...
#pragma warning disable CS1998 // Async method lacks 'await' operators and will run synchronously
    public async Task<bool> DeleteFileAsync(UploadedFile file, CancellationToken cancellationToken = default)
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
    {
    ...
    public /*async */ Task<string> GetFileUrl(UploadedFile file)
    {
        throw new NotImplementedException();
    }
}
